<template>
    <kPicker ref="picker" :show="show" :closeOnClickOverlay="closeOnClickOverlay" :columns="columns" :title="title"
...
import kPicker from "../k-picker/index"
export default {
    ...
    methods: {
        ...
        correctValue(value) {
            ...
            if (isDateMode && !this.date(value)) {
                // 如果是日期类型，但是又没有设置合法的当前时间的话，使用最小时间为当前时间
                value = this.minDate
            } else if (!isDateMode && !value) {
                // 如果是时间类型，而又没有默认值的话，就用最小时间
                value = `${this.padZero(this.minHour)}:${this.padZero(this.minMinute)}`
            }
            // 时间类型
            if (!isDateMode) {
                if (String(value).indexOf(':') === -1) return console.log('时间错误，请传递如12:24的格式')
                let [hour, minute] = value.split(':')
                // 对时间补零，同时控制在最小值和最大值之间
                hour = this.padZero(this.range(this.minHour, this.maxHour, Number(hour)))
                minute = this.padZero(this.range(this.minMinute, this.maxMinute, Number(minute)))
                return `${hour}:${minute}`
            } else {
